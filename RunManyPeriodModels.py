...
# filetype (usually constsust_32) to make a linear model of the target variable before
# calculating statistics. This controls for normal stock market activity.
...
# "OPEC_conf_28" (only 28 equally spaced OPEC meetings so stats are easier), "OPEC_Conference"
...
for filetype in filetypes:
    ...
    if plots_on:
        ...
        model = sm.OLS(bycop["geoSD"] * 100, sm.add_constant(bycop.reset_index(drop=True).index + 1))
        results = model.fit()
        LOBgrad = results.params["x1"]
        LOBFosset = results.params["const"]
        trendline_stats = pd.concat([
        ...
        plt.hist(histogramdf[histogramdf.offset == 0]["geoSD"], density=True,
                 bins=bin_edges)
        plt.hist(definitelynotcops.loc[definitelynotcops["geoSD"] < histlimit, "geoSD"],
                 density=True, bins=bin_edges, alpha=0.6)
        plt.xlabel("Geometric standard deviation")
        plt.ylabel("Density of instances")
        plt.legend([copOrOtherLongstring, "Other times"])
